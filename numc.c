...
static PyObject *Matrix61c_add(Matrix61c* self, PyObject* args) {
    ...
    int i = add_matrix(new_mat, self->mat, b->mat);
    if (i == -1) {
    ...
    Matrix61c* rv = (Matrix61c*) Matrix61c_new(&Matrix61cType, NULL, NULL);
    rv->mat = new_mat;
    rv->shape = PyTuple_Pack(2, PyLong_FromLong(rows), PyLong_FromLong(cols));
    return (PyObject*)rv;
}
...
static PyObject *Matrix61c_multiply(Matrix61c* self, PyObject *args) {
    ...
    int i = mul_matrix(new_mat, self->mat, b->mat);
    if (i == -1) {
        PyErr_SetString(PyExc_ValueError, "Incorrect dimensions!");
        return NULL; // Value Error
    }
...
static PyObject *Matrix61c_set_value(Matrix61c *self, PyObject* args) {
    ...
    PyObject *j = NULL;
    PyObject *val = NULL;
    if (PyArg_UnpackTuple(args, "args", 3, 3, &i, &j, &val)) {
        if (i != NULL && PyLong_Check(i) && j != NULL && PyLong_Check(j) && val != NULL && (PyLong_Check(val) || PyFloat_Check(val))) {
            int row_i = PyLong_AsLong(i);
            int col_j = PyLong_AsLong(j);
            if (row_i < 0 || row_i >= rows || col_j < 0 || col_j >= cols) {
                PyErr_SetString(PyExc_IndexError, "Index out of range");
                return NULL;
            }
            if (PyLong_Check(val)) {
                long value = PyLong_AsLong(val); // when val is int
                set(self->mat, row_i, col_j, value);
                return Py_BuildValue("");
            } else {
                double value = PyFloat_AsDouble(val); // when val is double
                set(self->mat, row_i, col_j, value);
                return Py_BuildValue("");
            }
        } else {
...
static PyObject *Matrix61c_get_value(Matrix61c *self, PyObject* args) {
    ...
    PyObject *i = NULL;
    PyObject *j = NULL;
    if (PyArg_UnpackTuple(args, "args", 2, 2, &i, &j)) {
        if (i != NULL && PyLong_Check(i) && j != NULL && PyLong_Check(j)) {
            int row_i = PyLong_AsLong(i);
            int col_j = PyLong_AsLong(j);
            if (row_i < 0 || row_i >= rows || col_j < 0 || col_j >= cols) {
                PyErr_SetString(PyExc_IndexError, "Index out of range");
            ...
            double num = get(self->mat, row_i, col_j);
